(function () { //minified functions
    ...
    2: [function (require, module, exports) {
        ...
        function initMap() { //set up the map/styles, initial route from geolocation to endpoint, places along the route, and initial directions.
            ...
            // KS 6/10/2025: Get parsed float values, if present
            const latInput = parseFloat(document.getElementById('lat')?.value); // KS 6/10/2025
            const lonInput = parseFloat(document.getElementById('lon')?.value); // KS 6/10/2025

            // KS 6/10/2025: Default to Los Angeles if values are invalid
            const defaultLat = 34.0522; // KS 6/10/2025
            const defaultLng = -118.2437; // KS 6/10/2025

            const lat = isNaN(latInput) ? defaultLat : latInput; // KS 6/10/2025
            const lng = isNaN(lonInput) ? defaultLng : lonInput; // KS 6/10/2025

            map.setCenter({ lat, lng }); // KS 6/10/2025
                                ...
                                geocoder.geocode({ //get address
                                    ...
                                    if (status == google.maps.GeocoderStatus.OK) { //use geocoder to convert dest to latlng object.
                                        ...
                                        directionsService.route(request, function (result, status) {
                                            if (status == "OK") {
                                                fetch("https://www.donttrip.org/donttrip/backend/dt_backend", {
                                                    method: "post",
                                                    headers: {
        ...
        function initialize() {
            ...
            // KS 6/10/2025: Default to 'us' if countryCode is not found or empty
            const countryCodeInput = document.getElementById('countryCode')?.value?.trim(); // KS 6/10/2025
            const countryCode = countryCodeInput ? countryCodeInput : 'us'; // KS 6/10/2025

            let options = {
                componentRestrictions: {
                    country: countryCode // KS 6/10/2025
                }
            }; // KS 6/10/2025
                        ...
                        geocoder
                            ...
                            .then(({
                                ...
                                function callback(place, status) {
                                    ...
                                    moreInfo.href = "https://www.donttrip.org/donttrip/client/place" +
                                        "?rating=" +
                                        place.rating +
                                        "&name=" +
